...
export default function (pi: ExtensionAPI) {
  ...
  // Returns { systemPrompt } to modify the system prompt for this turn
  pi.on("before_agent_start", async (event, ctx) => {
    currentCtx = ctx;

    if (!activeAgent) return; // No agent selected, use default

    if (activeAgent.systemPromptMode === "replace") {
      // Replace entire system prompt with agent's prompt
      return { systemPrompt: activeAgent.systemPrompt };
    } else {
      // Append agent's prompt to existing system prompt
  ...
  pi.registerCommand("agent", {
    ...
    handler: async (args, ctx) => {
      ...
      if (!args || !args.trim()) {
        ...
        activeAgent = agent;
        if (ctx.hasUI) ctx.ui.setStatus("agent", agent.name);
